...
public class TerrainGenerator : MonoBehaviour
{
    ...
    void GenerateTerrain()
    {
        ...
        //StartCoroutine("RenderMap(PerlinNoiseSmooth(mapArray, seed, 3), map, tileBase)");
        RenderMap(PerlinNoiseSmooth(mapArray, seed, 10), map, tileBase);
    }
    ...
    public void RenderMap(int[,] map, Tilemap tilemap, TileBase tile)
    {
        ...
        for (int x = 0; x < map.GetUpperBound(0) ; x++)
        {
            ...
            for (int y = 0; y < map.GetUpperBound(1); y++)
            {
                ...
                if (map[x, y] == 1)
                {
                    tilemap.SetTile(new Vector3Int(x-(int)(terrainWidth/2), y-(int)(terrainHeight/2), 0), tile);
                }
            }
        }
    }
